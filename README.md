...
- GitHub Actions build/test/publish pipeline for every push to `master`.

## Published Docker image

The GitHub Actions workflow builds and publishes this image on every push to `master`:
...
- Backend helper regression tests inside the built image.
...
python3 -m py_compile docker_main.py web/app.py channel.py twitch.py inventory.py constants.py web/auth.py
